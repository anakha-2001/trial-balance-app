...
The National Company Law Tribunal vide its order dated  9th May, 2019 confirmed the petition for the reduction of the share capital of the Company. The company pursuant to the order of the Hon'ble Tribunal discharged the dues to the shareholders whose shares were reduced by depositing the fund with an Escrow Account opened for the purpose and paying the shareholders out of this account by Bank Transfer or Draft or other mode as indicated by the respective shareholder with the Company. For the year ended 31st March 2024 the capital reduction liability payable to shareholders has been discharged to the extent of Rs. 92,320/-.`
    ...
    const calculateNote13 = (): FinancialNote => {
      ...
      const total = {
        current:
          retainedClosing.current + oci.current + generalReserve.current - 0.01,
        previous:
          retainedClosing.previous +
          oci.previous +
          generalReserve.previous -
          0.01,
      };
    ...
    const calculateNote29 = (): FinancialNote => {
      ...
      const yr5 = {
        current: note29_15.valueCurrent ?? 0,
        previous: note29_15.valuePrevious ?? 0,
      };
      const rectotal = {
        current: note29_16.valueCurrent ?? 0,
        previous: note29_16.valuePrevious ?? 0,
      };
      const less = {
 ...
 - protect the Company's financial investments, while maximising returns.`
    ...
    const processNode = (
      ...
      // }
        ...
        if (val1 && val2) {
          valueCurrent =
            op === "+"
              ? val1.current + val2.current
              : val1.current - val2.current;
          valuePrevious =
            op === "+"
              ? val1.previous + val2.previous
              : val1.previous - val2.previous;
        } else {
          valueCurrent = null;
          valuePrevious = null;
    ...
    return {
      ...
      incomeStatement: INCOME_STATEMENT_STRUCTURE.map((node) =>
        processNode(node, enrichedData, getAmount)
      ),
      cashFlow: CASH_FLOW_STRUCTURE.map((node) =>
        processNode(node, enrichedData, getAmount)
      ),
      notes: allNotes,
...
export const joinManualJEAndRenamedData = (
  ...
  return renamedData.map((row) => {
    ...
    return {
      ...row,
      amountCurrent: ((row.amountCurrent || 0) + currentAdjustment) as
        | number
        | undefined,
      amountPrevious: ((row.amountPrevious || 0) + previousAdjustment) as
        | number
        | undefined,
    };
  });
...
const FinancialStatements: React.FC<FinancialStatementsProps> = ({
  ...
  const [cashFlowEditorContainer, setCashFlowEditorContainer] =
    useState<HTMLElement | null>(null);
  const [editedCashFlow, setEditedCashFlow] = useState<
    HierarchicalItem[] | null
  >(null);
  ...
  const fetchDatabaseData = async (period1: string, period2: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/trial-balance/data?period1=${encodeURIComponent(
          period1
      ...
      const fvResponse = await fetch(
        "http://localhost:5000/api/financial-variables1"
      );
      if (!fvResponse.ok) {
  ...
  useEffect(() => {
    const fetchJEs = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/api/journal/updated"
        );
  ...
  const currentAmountKeys = useDatabase
    ? {
        amountCurrentKey: selectedPeriods?.period1 || "",
        amountPreviousKey: selectedPeriods?.period2 || "",
      }
  ...
  const handleSaveChanges = (updatedNotes: FinancialNote[]) => {
    ...
    // 1. === GET EDITED VALUE BY KEY (for numeric financialVar2) ===
    ...
    // 2. === UPDATE FINANCIAL VARIABLES (financialVar2) ===
    ...
    // 4. === UPDATE TEXT VARIABLES (textVar2) ===
    const generateTextVarPayload = () => {
      const textVarUploadData = textVar2.map((row) => {
        const { key } = row;
        const narrativeText =
      ...
      console.log("textVarUploadData", textVarUploadData);
      ...
      const updatedTables = textVar2
        .map((row) => {
          ...
          if (tableContent) {
            return {
              key,
              [currentAmountKeys.amountCurrentKey]: JSON.stringify(
                tableContent.rows
  ...
  const handleSaveCashFlow = (updatedCashFlow: HierarchicalItem[]) => {
    ...
    const extractKeyValuePairs = (
      ...
      const traverse = (itemList: HierarchicalItem[]) => {
        for (const item of itemList) {
          // Push item if it has a defined 'key'
    ...
    // Step 1: Extract key-value pairs from updatedCashFlow
    const extracted = extractKeyValuePairs(updatedCashFlow);

    // Step 2: Convert extracted pairs to a map for quick lookup
    ...
    // Step 3: Update financialVar2 only if the key exists in extractedMap
    const updatedFinancialVar2 = financialVar2.map((row) => {
    ...
    // Step 5: POST updated values to server
    fetch("http://localhost:5000/api/update-financial-vars", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(renamedForServer),
